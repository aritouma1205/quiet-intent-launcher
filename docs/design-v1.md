...
個人プロファイル内の起動可能なActivityを、アイコン＋アプリ名の縦リストで表示する。日本語ロケールの名前順、同名はパッケージ名・Activity名の順。複数の起動Activityがある場合も対象IDを区別する。
タップで起動、長押しで「行動に割り当て」「アプリ情報」。アンインストールはアプリ情報側のOS操作に任せる。
